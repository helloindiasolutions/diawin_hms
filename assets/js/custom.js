(function () {
  ...
  var headerSearchEl = document.getElementById("header-search");
  if (headerSearchEl) {
...
(function () {
  ...
  const scrollToActiveMenu = () => {
    ...
    // 1. Ensure the correct menu item is marked active (Sync with current URL)
    const menuLinks = document.querySelectorAll('.side-menu__item');
    menuLinks.forEach(link => {
      ...
      if (href && href !== 'javascript:void(0);' && href !== '#') {
        ...
        // Match exact path or sub-paths (for edit/view pages)
        if (normPath === normHref || (normHref !== "" && normHref !== "/" && normPath.startsWith(normHref + "/"))) {
          link.classList.add('active');

          // Expand all parent submenus (Only for vertical layout to avoid horizontal mess)
          const navLayout = document.documentElement.getAttribute('data-nav-layout');
          let parent = link.closest('.slide.has-sub');
          while (parent) {
            parent.classList.add('open', 'is-expanded');
            if (navLayout !== 'horizontal') {
              const subMenu = parent.querySelector('.slide-menu');
              if (subMenu) subMenu.style.display = 'block';
            }
            // Look for the next level up
            const upperSlide = parent.parentElement.closest('.slide.has-sub');
            if (upperSlide && upperSlide !== parent) {
              parent = upperSlide;
            } else {
              parent = null;
            }
    ...
    // 2. Identify the active element and scroll it into view
    // We target the .side-menu__item.active or any element with .active inside the menu
    const activeItem = document.querySelector('.side-menu__item.active') ||
      document.querySelector('.slide-menu .active') ||
      document.querySelector('.main-menu .active');

    if (activeItem && sidebarScroll) {
      setTimeout(() => {
        // Check if SimpleBar is being used (common in this template)
        if (window.SimpleBar) {
          const sbInstance = SimpleBar.instances.get(sidebarScroll);
          if (sbInstance) {
            const scrollElement = sbInstance.getScrollElement();
            const itemOffset = activeItem.getBoundingClientRect().top - sidebarScroll.getBoundingClientRect().top;
            const currentScroll = scrollElement.scrollTop;
            const targetScroll = currentScroll + itemOffset - (sidebarScroll.clientHeight / 2);
        ...
        // Fallback to native scrollIntoView if SimpleBar logic fails
        activeItem.scrollIntoView({
          behavior: 'smooth',
          block: 'center',
          inline: 'nearest'
        });
      }, 400); // Slight delay to ensure DOM and SimpleBar are ready
    }
  };
