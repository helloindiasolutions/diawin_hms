...
// Prevent submenu from closing when clicking leaf nodes (links without children)
document.addEventListener("click", (e) => {
  const clickedLink = e.target.closest(".side-menu__item");
  if (!clickedLink) return;
  
  const parentSlide = clickedLink.parentElement;
  const hasChildren = parentSlide && parentSlide.classList.contains("has-sub");
  
  // If it's a leaf node (no children), don't close the parent submenu
  if (!hasChildren) {
    e.stopPropagation();
  }
});
...
document.addEventListener("click", (e) => {
  ...
  if (!clickedInsideSidebar && window.innerWidth >= 992) {
    // Close all open submenus
    document.querySelectorAll(".slide.has-sub.open").forEach((slide) => {
      const submenu = slide.querySelector(".slide-menu");
