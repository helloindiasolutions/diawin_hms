    ...
    class SpaNavigation {
        ...
        async navigate(url, pushState = true) {
            ...
            try {
                ...
                // 7. Execute new scripts (only inline/page-specific)
        ...
        injectPageStyles(doc) {
            ...
            // Find all inline style tags in the head (after the main styles)
            const headStyles = Array.from(doc.querySelectorAll('head style'));

            // Look for page-specific styles (typically the last style tag in head)
            // This is where <?= $styles ?? '' ?> content appears
            headStyles.forEach((oldStyle, index) => {
