    ...
    const FormValidator = {
        ...
        setupForm(form) {
            ...
            inputs.forEach(input => {
                ...
                // Detect browser autofill via animation (Chrome/Firefox trigger this)
                input.addEventListener('animationstart', function (e) {
                    if (e.animationName === 'onAutoFillStart' || e.animationName.includes('autofill')) {
                        setTimeout(() => self.updateSubmitButton(form), 100);
                    }
                });
            });

            // Multiple strategies to detect autofill
            // Strategy 1: Immediate check after short delay
            setTimeout(() => {
                inputs.forEach(input => {
            ...
            // Strategy 2: Check again after longer delay for slower autofill
            setTimeout(() => this.updateSubmitButton(form), 500);

            // Strategy 3: Check when user focuses on form (they're about to interact)
        ...
        checkFieldSilent(input) {
            ...
            if (value && (type === 'tel' || input.name === 'mobile') && !this.patterns.tel.test(value)) return false;
